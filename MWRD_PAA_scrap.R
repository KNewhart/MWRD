...
glm.new.data <- glm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Aerobic.SRT + NSEC.Effluent.OP + NSEC.Effluent.NO3 + NSEC.Effluent.NO5 + NSEC.Effluent.Flow, data=nsec.online.ecoli.data)
visreg(glm.new.data)
```
...
rf.new.data <- randomForest::randomForest(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Aerobic.SRT + NSEC.Effluent.OP + NSEC.Effluent.NO3 + NSEC.Effluent.NO5 + NSEC.Effluent.Flow, data=new.data)

svm.new.data <- e1071::svm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Aerobic.SRT + NSEC.Effluent.OP + NSEC.Effluent.NO3 + NSEC.Effluent.NO5 + NSEC.Effluent.Flow, data=new.data)

png("MWRD_Ecoli_nseconline_rf_svm.png", units = "in", res = 200, width = 2*2*3, height = 2*3, family="A", pointsize = 16)
par(mfrow=c(1,2), mar=c(4.5,4,.5,2))
visreg(fit=rf.new.data, xvar="NSEC.Aerobic.SRT", plot=TRUE, xlab = "North Secondary Aerobic SRT (d)", ylab = "Pre-disinf E. coli (LOG MPN/100mL)", points.par=c(col="black"), line.par=c(col="blue"), xlim=c(4,12), ylim=c(8,12))
text(x=12,y=12,labels="(a)", font=2)

visreg(fit=svm.new.data, xvar="NSEC.Aerobic.SRT", plot=TRUE, xlab = "North Secondary Aerobic SRT (d)", ylab = "Pre-disinf E. coli (LOG MPN/100mL)", points.par=c(col="black"), line.par=c(col="blue"), xlim=c(4,12), ylim=c(8,12))
text(x=12,y=12,labels="(b)", font=2)
dev.off()


png("MWRD_Ecoli_nseconline_svm.png", units = "in", res = 200, width = 2*3, height = 2*3, family="A")
par(mar=c(4.5,4.5,3,.5), cex.main = 1)
visreg2d(fit=svm.new.data, "NSEC.Aerobic.SRT", "NSEC.Effluent.NO5", plot.type="image", xlab = "North Secondary Aerobic SRT (d)", ylab = "North Secondary Effluent NO5", main = "Pre-disinf E. coli (LOG MPN/100mL)", ylim=c(0,60), xlim=c(4,10))
dev.off()
```
![](MWRD_Ecoli_nseconline_rf_svm.png)
**Figure .** Comparison of (a) random forest model and (b) support vector machine model for predicting *E. coli* into the PAA disinfection basin

![](MWRD_Ecoli_nseconline_svm.png)
**Figure .** Support vector machine model for predicting *E. coli* into the PAA disinfection basin

## Generalized Additive Models
```{r include=FALSE}
gam.new.data <- gam(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Aerobic.SRT + NSEC.Effluent.OP + NSEC.Effluent.NO3 + NSEC.Effluent.NO5 + NSEC.Effluent.Flow, data=new.data)
summary(gam.new.data)
```
