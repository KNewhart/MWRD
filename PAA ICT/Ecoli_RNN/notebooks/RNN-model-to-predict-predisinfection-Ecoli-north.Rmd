...
```{r}
source("src/compile-data.R")
instant.data <- compile.data("data/north/instant")
average.data <- compile.data("data/north/average")
average24.data <- compile.data("data/north/average-24")
```
...
instant.data <- na.locf(instant.data)[-c(1:3),]
remove <- instant.data %>% apply(., 2, anyNA) %>% unlist %>% which
if(length(remove) > 0) instant.data <- instant.data[,-remove]

average.data <- na.locf(average.data)[-c(1:3),]
remove <- average.data %>% apply(., 2, anyNA) %>% unlist %>% which
if(length(remove) > 0) average.data <- average.data[,-remove]

average24.data <- na.locf(average24.data)[-c(1:3),]
remove <- average24.data %>% apply(., 2, anyNA) %>% unlist %>% which
if(length(remove) > 0) average24.data <- average24.data[,-remove]
```
...
for(i in 1:ncol(average.data)) {
  png(filename=paste0("figures/north/average/",gsub("[.]","-",colnames(average.data)[i]),".png"))
  par(mar=c(2,2.5,2.5,.1), cex.main=1)
  plot(average.data[,i], ylab="", xlab="", main=colnames(average.data)[i])
  dev.off()
}
...
for(i in 1:ncol(average24.data)) {
  png(filename=paste0("figures/north/average-24/",gsub("[.]","-",colnames(average24.data)[i]),".png"))
  par(mar=c(2,2.5,2.5,.1), cex.main=1)
  plot(average24.data[,i], ylab="", xlab="", main=colnames(average24.data)[i])
  dev.off()
}
...
for(i in 1:ncol(instant.data)) {
  png(filename=paste0("figures/north/instant/",gsub("[.]","-",colnames(average.data)[i]),".png"))
  par(mar=c(2,2.5,2.5,.1), cex.main=1)
  plot(average.data[,i], ylab="", xlab="", main=colnames(average.data)[i])
  dev.off()
}
...
test.average <- list(test.average.1, test.average.2, test.average.3)
names(test.average) <- c("250", "500", "750")
save.results(test.average, obj.folder="Over-under fit",obj.name="average-log-lag-north")
...
test.average <- list(test.average.4, test.average)
names(test.average) <- c("100", "250", "500", "750")
save.results(test.average, obj.folder="Over-under fit",obj.name="average-log-lag-north")
```
