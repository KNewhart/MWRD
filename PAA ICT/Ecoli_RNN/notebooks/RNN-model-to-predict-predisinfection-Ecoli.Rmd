...
## Compare average vs 24h average
```{r}
source("src/rolling-rnn.R")
average.data.log.lag <- average.data
average.times <- index(average.data)
...
average24.data.log.lag <- average24.data
average24.times <- index(average24.data)
predict.col <- which(colnames(average24.data.log.lag)=="ECIDX_G") # Select Ecoli column
average24.data.log.lag[,predict.col] <- log10(average24.data.log.lag[,predict.col]) # Log transform Ecoli
average24.data.log.lag <- cbind(average24.data.log.lag[3:nrow(average24.data.log.lag),], 
                              as.numeric(average24.data.log.lag[1:(nrow(average24.data.log.lag)-2),predict.col]))
colnames(average24.data.log.lag)[ncol(average24.data.log.lag)] <- "ECIDX_G_Lag"
average24.data.log.lag <- average24.data.log.lag[which(!apply(average24.data.log.lag, 1, anyNA)),]

test.average.1 <- rolling.rnn(all.data=average.data.log.lag,
                            predict.col=which(colnames(average.data.log.lag)=="ECIDX_G"),
                            train.obs=ceiling(nrow(average.data.log.lag)*.9),
                            n_epoch=150)
test.average.2 <- rolling.rnn(all.data=average24.data.log.lag,
                            predict.col=which(colnames(average24.data.log.lag)=="ECIDX_G"),
                            train.obs=ceiling(nrow(average24.data.log.lag)*.9),
                            n_epoch=150)
```
