...
# Compare results:
lapply(list(test.average.1, test.average.2, test.average.3), function(results.list) {
  lapply(results.list, function(results) {
    paste("R2 =",mean(results[,1]),", RMSE =",mean((results[,2]-results[,3])^2)^0.5)
  })
})

l <- lapply(list(test.average.1, test.average.2, test.average.3), function(results.list) {
