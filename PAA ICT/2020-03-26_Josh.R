...
# time.index <- lubridate::round_date(index(all.data), "min")
# time.index <- sapply(unique(time.index), function(t) tail(which(time.index==t),n=1))
# all.data <- xts(all.data[time.index,], order.by = lubridate::round_date(index(all.data)[time.index], "min"))
...
# We see high applied CT (>10) when HRT is high
par(mfrow=c(1,1), mar=c(4,4,1,4.5))
