...
rollingANN <- function(all.data,
  ...
  p <- colnames(all.data)[predict.col]
  ...
  while(start <= nrow(all.data)) {
    ...
    predictions <- foreach(test.obs=seq(start, end),
                           .packages = c("dplyr", "keras","zoo"), .export=c("min.max.norm")) %dopar% {
                             ...
                             # if(scale) train.x <- scale(all.data[train.start:train.end,], center=train.mean, scale=train.sd)[,-predict.col]
                             ...
                             if(length(dim(train.x)) == 3) {
                               # if(scale) train.y <- simplify2array(list(scale(all.data[train.start:train.end,], center=train.mean, scale=train.sd)[,predict.col]))
                               if(scale) train.y <- simplify2array(list(min.max.norm(all.data[train.start:train.end,])[,predict.col]))
                               if(!scale) train.y <- simplify2array(list(all.data[train.start:train.end,predict.col]))
