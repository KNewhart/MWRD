...
for(i in 1:length(paa.data.ls)) {
  ...
  # for(j in 16:ncol(combos)) { # interupted, bad col removed
  ...
  rmse.results <- unlist(lapply(combo.results.ls, function(set) mean(unlist(lapply(set, function(trial) mean((trial[,2]-trial[,3])^2)^0.5)))))
  previous.rmse <- min(rmse.results)
  current.rmse <- min(rmse.results)
  while(current.rmse <= previous.rmse) {
    previous.rmse <- min(rmse.results) 
    previous.vars <- combos[,which(rmse.results==min(rmse.results))]
...
if(!("results/ecoli-var-results-ls.RData" %in% list.files(recursive=TRUE))) {
...
for(i in 1:length(ecoli.data.ls)) {
  ...
  # save(combo.results.ls,file="testing.RData")
  all.results.ls[[length(all.results.ls)+1]] <- previous.results  # Results of all combos for 2018/2019
  print(paste("Finished",i,"year"))
}

save(all.results.ls, file="results/ecoli-var-results-ls.RData")
} else {
