...
for(i in 1:length(paa.data.ls)) {
  ...
  for(j in 1:length(combos)) {
    ...
    for(k in 1:ncol(paa)) {
      ...
      resultsANN.k[[length(resultsANN.k)+1]] <- resultsANN.n # 10 iterations of a combo for a sampling location
      
    }
    combo.results.ls[[length(combo.results.ls)+1]] <- resultsANN.k # Results of a combo for all sampling locations
  } 
  ###### CHECK RMSE AND WRITE CONDITIONAL STATEMENT TO REMOVE WORSE COMBO ######
  # save(combo.results.ls,file="testing.RData")
  all.results.ls[[length(all.results.ls)+1]] <- combo.results.ls # Results of all combos for 2018/2019
}
end <- Sys.time()
end-start
...
```{r Build Ecoli RNN, eval=FALSE, include=FALSE}
if(!("results/ecoli-rnn-ann-all-results-ls.RData" %in% list.files(recursive=TRUE))) { 
...
save(all.results.ls, file="results/ecoli-rnn-ann-all-results-ls.RData")
} else {
  load("results/ecoli-rnn-ann-all-results-ls.RData")
}
```