...
if(!("results/paa-all-results-ls-lablag.RData" %in% list.files(recursive=TRUE))) {
  load("data/paa/process-data-ls-lablag.RData")
  start <- Sys.time()
all.results.ls <- list()
# 5. Merge data for testing
...
save(all.results.ls, file="results/paa-all-results-ls-lablag.RData")
} else {
...
plot.results$loc <- factor(plot.results$loc, levels=c("2018 1-min", "2019 1-min", "2019 10-min", 
                                                        "2018 Half", "2019 20-min", "2019 30-min"))
...
```
```{r Timeseries PAA prediction actual v2, fig.height=2.5, fig.width=7.5}
load("results/paa-all-results-ls-lablag.RData")

actual.predicted.plot <- function(actual, predicted, label) {
  par(mar=c(2, 2, 3, 0) + 0.1, family="serif")
  # r <- c(min(c(actual[,2], predicted[,2])), max(c(actual[,2], predicted[,2])))
  r <- c(0,1.5)
  plot(x=actual[,1], y=actual[,2], main=label, pch=20,xlab="", ylab="", ylim=c(r[1], r[2]), xaxt="n", lwd=1.75)
  times <- as.POSIXct(actual[,1], origin="1970-01-01")
  r <- as.POSIXct(round(range(times), "days"))
  axis.POSIXct(1, at = seq(r[1], r[2], by = "week"), format = "%b %d")
  points(x=predicted[,1], y=predicted[,2], pch=20, col="purple", lwd=1.75)
  line.matrix <- matrix(data=c(actual[,1], actual[,2], predicted[,1], predicted[,2]), ncol=4)
  sapply(1:nrow(actual), function(i) lines(x=line.matrix[i,c(1,3)], y=line.matrix[i,c(2,4)]))
  # legend("topleft", legend=c("Actual", "Predicted"), pch=20, col=c("black", "purple"))
  mtext(text=paste0("RMSE=",sprintf("%.3f",round(mean((actual[,2]-predicted[,2])^2)^0.5,3))), side=3, adj=0.5, line=0.1)
}
# png("figures/Timeseries PAA prediction actual v2-1.png", width=7.5, height=2.5, units="in", res=600)
all.loc.ls <- list()
par(mfcol=c(1,4), cex=1, cex.axis=1, cex.lab=1, cex.main=1, cex.sub=1)
for(avg in 1) {
  avg.lab <- c("Instantaneous", "Average", "24-hr Avg")
  for(year in 2) {
    year.lab <- c("2018", "2019")
    for(loc in 1:length(all.results.ls[[year]][[avg]])) {
      ...
      actual.predicted.plot(actual=cbind(times, actual),
                            predicted=cbind(times, predicted),
                            label=paste("Location",loc))
      all.loc.ls[[length(all.loc.ls)+1]] <- cbind(times, actual, predicted)
    }
  }
}
# dev.off()
...
```{r Calculate Persistence CT for 2019, echo=FALSE, fig.width=6.5, fig.height=6.5}
load("results/ct-all-results-ls.RData")
load("data/paa/process-data-ls-lablag.RData")
load("results/ct-ann-results-ls-lablag.RData")
...
C0.ls <- list()
n <- 1
par(family="serif", mar=c(2.5,3.25,0.6,0.6), mfrow=c(3,2), oma=c(.1,.1,1.5,0))
se.ls <- list()
for(i in 1:length(all.results.ls)) {
  ...
  plot(x=x, y=y, pch=17, ylab="", xlab="", xaxt="n", ylim=c(0,max(-unlist(lapply(all.results.ls, function(x) x[,2])))), cex=0.8)
  if(i==1) year <- "2018"
  if(i==2) year <- "2019"
  ...
  se <- sapply(1:length(y), function(z) (y[z]-avg.k[[i]])^2)
  mtext(side=3, line=-1.1, adj=0.01, text=round(mean(se)^0.5,2)) # Persistence
  
  se.ls[[length(se.ls)+1]] <- se
}
...
for(i in 1:length(all.results.ls)) {
  ...
  plot(x=x, y=y, pch=17, ylab="", xlab="", xaxt="n", ylim=c(0,2), cex=0.8)
  axis(side=1, at = lab.tic, 
       labels = FALSE)
  m <- 8
  if(length(lab.tex) < 15) m <- 3
  ...
  if(i==1) {
    ...
    se <- sapply(1:length(y), function(z) (y[z]-avg.D[[i]])^2)
    mtext(side=3, line=-1.1, adj=0.01, text=round(mean(se)^0.5,2))
    se.ls[[length(se.ls)+1]] <- se
  }
  if(i ==2) {
    points(x=x, y=D, col="red", cex=1, pch=20)
    abline(h=mean(y), lty=2, col="red", lwd=2)
    se <- sapply(1:length(y), function(z) (y[z]-D[z])^2)
    mtext(side=3, line=-1.1, adj=0.01, text=round(mean(se)^0.5,2))
    se.ls[[length(se.ls)+1]] <- se
  }
...
for(i in 1:length(all.results.ls)) {
  ...
  plot(x=x, y=y, pch=17, ylab="", xlab="", xaxt="n", cex=0.8, ylim=c(5,91))
  axis(side=1, at = lab.tic, 
       labels = FALSE)
  m <- 8
  if(length(lab.tex) < 15) m <- 3
  ...
  points(x=x, y=CT, col="red", cex=1, pch=20) # Persistence
  predicted <- rowMeans(do.call("cbind",lapply(ct.results.ls[[i]][[1]], function(x) x[,2])))
  points(x=x, y=predicted, col="cornflowerblue", cex=0.65, pch=15)
  points(x=x, y=y, pch=17, cex=0.8)
  ...
  se <- sapply(1:length(y), function(z) (y[z]-CT[z])^2)
  mtext(side=3, line=-1.1, adj=0.01, text=round(mean(se)^0.5,2)) # Persistence
  se.ls[[length(se.ls)+1]] <- se
  mtext(side=3, line=-2.1, adj=0.01, text=round(mean(sapply(1:length(y), function(z) (y[z]-predicted[z])^2))^0.5,2)) # Prediction
  print(((mean(se)^0.5)-(mean(sapply(1:length(y), function(z) (y[z]-predicted[z])^2))^0.5))/(mean(se)^0.5))
}

```

```{r Boxplots of CT D k, echo=FALSE, fig.width=4, fig.height=6}
load("results/ct-all-results-ls.RData")
layout(mat=matrix(c(1,2,3,4,5,6), ncol=2, byrow = TRUE))
par(oma=c(0,2.5,2,0), family="serif")
par(mar=c(1,2,1,1))
for(i in 1:length(all.results.ls)) {
...
load("results/ct-all-results-ls.RData")
all.results.ls.ct <- all.results.ls
load("results/paa-all-results-ls-lablag.RData")
load("data/paa/process-data-ls-lablag.RData")
i <- 1;j <- 1
plot.these <- list(c(156),
                   c(11))
results.ls <- list()
r2.ls <- list()
anova.ls <- list()
# 5. Merge data for testing
year <- c("2018", "2019")
par(mfrow=c(1,1), mar=c(c(4, 4, 2.5, 1) + 0.1), family="serif")
# par(mfrow=c(3,3), mar=c(c(4, 4, 2.5, 1) + 0.1), family="serif")
for(i in 1:length(paa.data.ls)) {
  paa <- paa.data.ls[[i]] # 2018 or 2019
  ...
  for(j in 1:dim(results)[1]) {
    ...
    ## ADD conventional and conventional+analyzer
    C0.c <- C0.ls[[i]][j]
    D.c <- D.ls[[i]]
    if(length(D.c)>1) D.c <- D.c[j]
    k.c <- -1*avg.k[[i]]
    old.data <- data.frame(x=seq(0, t_f[n], length.out=100))
    old.data <- cbind(old.data, 
                      (C0.c-D.c)*exp(-k.c*old.data))
    points(x=old.data[,1],y=old.data[,2], type="l", lty=2, lwd=2, col="blue")
      
    mtext(paste("Conventional CT =", round((C0.c-D.c)/k.c-(C0.c-D.c)/k.c*exp(-k.c*t_f[n]),1)), side=3,line=-3.4, adj=0.99)
    }
...
lapply(results.ls, function(results) which(min(abs(results[,4]-results[,6]))==abs(results[,4]-results[,6])))
wilcox.test(results[,4], results[,6], paired = TRUE)

wilcox.test(r2.ls[[2]][,1], r2.ls[[2]][,2], paired = TRUE, alternative="less")
median(r2.ls[[2]][,1])
median(r2.ls[[2]][,2])

v <- which(results.ls[[2]][,7]>45)
wilcox.test(r2.ls[[2]][v,1], r2.ls[[2]][v,2], paired = TRUE, alternative="less")
median(r2.ls[[2]][v,1])
median(r2.ls[[2]][v,2])
```
...
if(!("results/ct-ann-results-ls-lablag.RData" %in% list.files(recursive=TRUE))) {
  ...
  save(ct.results.ls, file="results/ct-ann-results-ls-lablag.RData")
} else {
  load("results/ct-ann-results-ls-lablag.RData")
}
```
...
# for(i in 2) {
  ...
  while(previous.rmse >= current.rmse) {
    ...
    # Train all combinations n times
      for(j in 1:ncol(combos)) {
        t1 <- Sys.time()
          ...
          combo.results.ls[[length(combo.results.ls)+1]] <- resultsANN.n # n iterations of a combo for a sampling location
          save(combo.results.ls, file="results-temp-combo-results-ls-ct.RData")
          print(paste(Sys.time(),"Combo", j, "of", ncol(combos), "completed"))
          t2 <- Sys.time()
    ...
    # Save results
    results.ls[[length(results.ls)+1]] <- list(combos, rsq, rmse)
    save(results.ls, file="results/temp-results-ls-ct.RData")
    # Compare to previous
    previous.rmse <- current.rmse
    current.rmse <- min(rmse)
  ...
  save(all.results.ls, file="results/temp-all-results-ls-ct.RData")
}
...
```{r Var CT exploratory, eval=FALSE, include=FALSE}
...
all.results.ls <- list(get(load("results/temp-all-results-ls-ct.RData")))
# load("results/ct-var-results-ls.RData")
...
for(j in 1:(optimum.run+1)) {
  ...
  var.df[j,1] <- var.names[n-1]
  # var.df[i,2] <- all.results.ls[[ecoli.i]][[1]][[3]][which(all.results.ls[[ecoli.i]][[1]][[1]]==n)]
  var.df[j,2] <- optimum.rmse
  var.df[j,3] <- i
}

# ct <- c(8.585191, 8.222428, 8.167425,7.942821,7.753953,7.674088,7.695667)
# ct.lab <- c("X2515.NSEC.Quad.4.MLR.SVI_G","AI_N99C","NSEC.INF..FY.F25.",
#             "TI.R3003","HRT.2018.2","ASRT_ASRT_N","NSEC.EFF.FLOW..FY-F225.")
ct <- var.df[,2]
ct.lab <- var.df[,1]

lab.fix <- function(lab) {
  name.mat <- list("RAS %",expression("2"*degree~"RAS (%)"),
                            "BOD","Final BOD",
                            "TSS", expression("2"*degree~"Eff TSS"),
                            "ASRT", expression("2"*degree~"ASRT"), 
                            "AI_N92C", expression("2"*degree~"NO"[2]),
                            "COD", expression("2"*degree~"COD"),
                            "AI_N99C", expression("2"*degree~"TSS"),
                            "TN_FC24", expression("2"*degree~"Eff TN"),
                            "AI_N93D", expression("2"*degree~"pH"),
                            "TI.R3003", "Air Temperature",
                            "FY.F25", expression("2"*degree~"Inf Flow"),
                            "F225", expression("2"*degree~"Eff Flow"),
                            "FI_T632", "GTE to SAR Flow",
                            "TI_N171", expression("2"*degree~"Temperature"),
                            "AC_N94C", expression("2"*degree~"DO"),
                            "HRT", "HRT",
                            "LI_N561Q", expression("2"*degree~"Blanket Depth"),
                            "Quad.4.MLR.SVI_G", "MLR SVI 4",
                            "Quad.3.MLR.SVI_G", "MLR SVI 3",
                            "Quad.2.MLR.SVI_G", "MLR SVI 2",
                   "Quad.1.MLR.SVI_G", "MLR SVI 1",
                            "AC_N94A", expression("2"*degree~"DO"),
                            "North.Flow.to.Dis", "Disinfection Flow",
                   "PAA_N_FlowRate", "PAA Flow",
                   "SACto", "SAC",
                   "RAS", "Total RAS Flow")
  for(i in seq(1,length(name.mat),2)) {
    name <- name.mat[[i]]
...
plot(x=1:(length(ct)), y=ct[1:(length(ct))], 
     ylim=c(min(ct-.5), max(ct)+.5),
...
pos.v <- c(rep(4,5),
             2,4,2,1,3,
             1)
# pos.v <- c(3,1,3,2,3,1,3)
offset.v <- sapply(pos.v, function(x) {
  if((x==1)||(x==3)) return(0.5)
  if((x==2) || (x==4)) return(1)
})
...
mtext(stats.str[1], side=3, adj=0.04, line=-1)
mtext(stats.str[2], side=3, adj=0.04, line=-2)
predicted.ct <- predicted

wilcox.test(predicted.paa-actual, predicted.ct - actual, paired = TRUE)
wilcox.test(predicted.paa, predicted.ct, paired = TRUE)
```
...
```{r chick-watsom kinetic model, eval=FALSE, fig.height=3, fig.width=3.75, include=FALSE}
library(xts)
load("data/ecoli/ecoli-data-ls.RData")
load("results/ct-all-results-ls.RData")
ecoli.index <- sapply(1:nrow(ecoli.data.ls[[2]]), function (n) {
...
CT <- do.call("rbind",lapply(all.results.ls, function(x) x[,4]))
CT.index <- sapply(1:nrow(log.removal), function(n) {
  m <- which(min(abs(difftime(index(log.removal)[n],
                              index(CT))))==abs(difftime(index(log.removal)[n],
                                                         index(CT))))
  return(m)
})
...
t <- as.numeric(do.call("rbind", results.ls[[i]])[CT.index,7])
CT.x <- as.numeric(CT[CT.index])
...
```{r Ecoli CT Predis 3D, eval=FALSE, fig.width=6.5, include=FALSE}
...
```{r Actual Log Inactivation vs Predicted}
library(xts)
load("data/ecoli/ecoli-data-ls.RData")
ecoli.index <- sapply(1:nrow(ecoli.data.ls[[2]]), function (n) {
  m <- which(as.Date(ecoli.data.ls[[1]][,1])==as.Date(ecoli.data.ls[[2]][n,1]))
...
log.removal <- log10(ecoli.data.ls[[2]][,2]/ecoli.data.ls[[1]][ecoli.index,2])
log.removal <- xts(log.removal, order.by=ecoli.data.ls[[2]][,1])
log.removal <- log.removal[3:length(log.removal)]
removal <- 10^log.removal

load("results/ecoli-ann-all-results-ls.RData")
pre.pred <- all.results.ls[[1]][[3]]
pre.pred <- lapply(1:nrow(pre.pred[[1]]), function(i) lapply(pre.pred, function(x) x[i,2]))
pre.pred <- unlist(lapply(pre.pred, function(x) mean(unlist(x))))
load("results/ecoli-ann-all-results-ls.RData")
post.pred <- all.results.ls[[2]][[3]]
post.pred <- lapply(1:nrow(post.pred[[1]]), function(i) lapply(post.pred, function(x) x[i,2]))
post.pred <- unlist(lapply(post.pred, function(x) mean(unlist(x))))

log.removal.pred <- log10((10^post.pred)/(10^pre.pred[ecoli.index[3:length(ecoli.index)]]))
log.removal.pred <- xts(log.removal.pred, order.by=index(log.removal))
removal.pred <- 10^log.removal.pred

plot(x=as.numeric(log.removal)*-1, y=as.numeric(log.removal.pred)*-1, pch=20, xlab="Actual", ylab="Predicted", xlim=c(2,9), ylim=c(2,9));abline(a=0,b=1)
plot(x=as.numeric(ecoli.data.ls[[1]][3:nrow(ecoli.data.ls[[1]]),2]), y=as.numeric(10^pre.pred), pch=20, xlab="Actual", ylab="Predicted");abline(a=0,b=1)
plot(x=as.numeric(ecoli.data.ls[[2]][3:nrow(ecoli.data.ls[[2]]),2]), y=as.numeric(10^post.pred), pch=20, xlab="Actual", ylab="Predicted");abline(a=0,b=1)

x=as.numeric(ecoli.data.ls[[2]][3:nrow(ecoli.data.ls[[2]]),2])
y=as.numeric(10^post.pred)
lm(y~x)
```

```{r Variable selection ANN Ecoli, eval=FALSE, include=FALSE}
if(!("results/ecoli-var-results-ls-n.RData" %in% list.files(recursive=TRUE))) {
...
lagged.times <- sapply(index(ecoli), function(t) {
  ...
  while(previous.rmse >= current.rmse) {
    ...
    # Train all combinations n times
      ...
      for(j in 1:ncol(combos)) {
          ...
          combo.results.ls[[length(combo.results.ls)+1]] <- resultsANN.n # n iterations of a combo for a sampling location
          save(combo.results.ls, file="results/temp-combo-results-ls.RData")
          print(paste(Sys.time(),"Combo", j, "of", ncol(combos), "completed"))
          t2 <- Sys.time()
          print(paste("Estimated time to combo completion:", Sys.time()+as.numeric(difftime(t2,t1, units="secs"))*(ncol(combos)-j)))
      }
    ...
    # Save results
    results.ls[[length(results.ls)+1]] <- list(combos, rsq, rmse)
    save(results.ls, file="results/temp-results-ls-ecoli.RData")
  ...
  all.results.ls[[length(all.results.ls)+1]] <- results.ls
  save(all.results.ls, file="results/temp-all-results-ls-ecoli.RData")
}

save(all.results.ls, file="results/ecoli-var-results-ls-n.RData")
} else {
  load("results/ecoli-var-results-ls-n.RData")
}
```
...
load("results/ct-all-results-ls.RData")
CT <- do.call("rbind",lapply(all.results.ls, function(x) x[,4]))
load("results/ecoli-var-results-ls.RData")
var.df.ls <- list()
...
lagged.times <- sapply(index(ecoli), function(t) {
  ...
  merged.data <- merged.data[,apply(merged.data, 2, function(x) length(unique(x))>3)]
    ...
    # Initialize loop
    vars <- which(!(colnames(merged.data) %in% predict.var))
    var.names <- colnames(merged.data)[vars]
    combos <- combn(vars,1) # Initialize
}
...
pre.lab <- var.df[which(var.df$Label==1),1]
post.lab <- var.df[which(var.df$Label==2),1]
...
pre.lab <- sapply(pre.lab, function(lab) lab.fix(lab))
post.lab <- sapply(post.lab, function(lab) lab.fix(lab))

# Plot RMSE as variables are added

pdf(file="figures/var-select-ecoli.pdf", width=6.5, height=3)
par(mar=c(3.5,3.5,1,1))
plot(x=c(1:length(pre)), y=pre, ylim=c(min(c(pre,post)-.0025), max(c(pre,post))), pch=20, type="b", xaxt="n", ylab="", xlab="", xlim=c(0.5,length(pre)+.5))
lines(x=c(1:length(post)), y=post, type="b", col="blue", pch=20)
mtext("RMSE", side=2, line=2.25)
mtext("Iteration", side=1, line=2.25)
axis(side=1, at = c(1:5), labels = c(1:5))
legend("topright", legend=c("Predisinfection E. coli", "Postdisinfection E. coli"), col=c("black", "blue"), pch=20)

for(x in 1:length(pre)) {
  y <- pre[x]
  text(pre.lab[x],x=x,y=y,pos=1)
}

for(x in 1:length(post)) {
  y <- post[x]
  text(post.lab[x],x=x,y=y,pos=1, col="blue")
}
dev.off()
```
...
# Plot without x axis
boundaries <- boxplot(plot.data, xaxt = "n", pch=20, ylim=c(0.09,0.30), col=c("pink", "pink", "bisque2", "bisque2", "cornflowerblue", "cornflowerblue"))
mtext(titles[i], side=3, line=0.5, font=2)
mtext("RMSE", side=2, line=2.25)
axis(side=1,at=1:6,labels=FALSE)
text(labels=colnames(plot.data), x=1:6, y=rep(0.04,6), xpd=TRUE, srt = 30, adj = 1,)
abline(h=results.p[i,1], col="red", xpd=FALSE)
}



# Instant RNN prediction as predisinfection E.coli
# Avearge RNN prediction for postdisinfection E.coli
```
