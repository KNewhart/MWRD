...
# Save E coli data
write.csv(master.file, file="data/postdisinfection-ecoli/south/postdisinfection-ecoli.csv")
...
foreach(p=1:length(tag.names),.packages = c("xts", "lubridate", "piwebapi")) %dopar% {
  ...
  write.csv(cbind(master.timestamps, file), file=paste0("data/postdisinfection-ecoli/south/instant/",tag.names[p],".csv"))
  write.csv(cbind(master.timestamps, file.avg), file=paste0("data/postdisinfection-ecoli/south/average/",tag.names[p],".csv"))
  write.csv(cbind(master.timestamps, file.avg.24), file=paste0("data/postdisinfection-ecoli/south/average-24/",tag.names[p],".csv"))
}
...
instant.data <- compile.data("data/postdisinfection-ecoli/north/instant")
average.data <- compile.data("data/postdisinfection-ecoli/north/average")
average24.data <- compile.data("data/postdisinfection-ecoli/north/average-24")
```
...
instant.data.log.lag <- instant.data
instant.times <- index(instant.data)
predict.col <- which(colnames(instant.data.log.lag)=="ECIDX_G") # Select Ecoli column
instant.data.log.lag[,predict.col] <- log10(instant.data.log.lag[,predict.col]) # Log transform Ecoli
instant.data.log.lag <- cbind(instant.data.log.lag[3:nrow(instant.data.log.lag),], 
                              as.numeric(instant.data.log.lag[1:(nrow(instant.data.log.lag)-2),predict.col]))
colnames(instant.data.log.lag)[ncol(instant.data.log.lag)] <- "ECIDX_G_Lag"
instant.data.log.lag <- instant.data.log.lag[which(!apply(instant.data.log.lag, 1, anyNA)),]

# Identify best n_epoch: 
# Are we overfitting? Underfitting?
test.average <- list()
for(i in 1:10) {
  test.average[[length(test.average)+1]] <- rolling.rnn(all.data=instant.data.log.lag,
                                                        predict.col=which(colnames(instant.data.log.lag)=="ECIDX_G"),
                                                        train.obs=ceiling(nrow(instant.data.log.lag)*.9),
                                                        n_epoch=250)
}
source("src/save-results.R")
save.results(test.average, obj.folder="Over-under fit",obj.name="instant-log-lag-south-postdis")
...
source("src/save-results.R")
save.results(test.average, obj.folder="Over-under fit",obj.name="average-log-lag-south-postdis-2")
```
