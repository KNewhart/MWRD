...
# for(i in 2) { # Test on 2019 dataset first
  ...
  for(k in 1:ncol(paa)) { # For each sampling location
    ...
    all.results.ls[[length(all.results.ls)+1]] <- combo.results.ls
    # Save results
  ...
  all.combo.results.ls[[length(all.combo.results.ls)+1]] <- all.results.ls
...
new.results <- array(data=NA, dim=c(6, 4, 3))
...
for(avg in 1:3) {
  for(year in 1:length(all.results.ls)) {
    for(loc in 1:length(all.results.ls[[year]][[avg]])) {
      ...
      new.results[r,1,avg] <- mean(unlist(lapply(data, function(x) x[,1]))) # Mean R2
      # new.results[r,2,avg] <- mean(unlist(lapply(data, function(x) x[,4]))) # Mean AIC
      # new.results[r,3,avg] <- mean(unlist(lapply(data, function(x) x[,5]))) # Mean BIC
      new.results[r,4,avg] <- mean(unlist(lapply(data, function(x) (x[,2]-x[,3])^2)))^0.5 # rmse
...
format(apply(new.results,1,function(x) x[,1]), scientific=FALSE)
...
```{r}
load("results/ct-ann-results-ls-v2.RData")
new.results <- array(data=NA, dim=c(length(ct.results.ls), 4, 3))

r <- 1
for(avg in 1:3) {
  for(year in 1:length(ct.results.ls)) {
      data <- ct.results.ls[[year]][[avg]]
      new.results[r,1,avg] <- mean(unlist(lapply(data, function(x) x[,1]))) # Mean R2
      # new.results[r,2,avg] <- mean(unlist(lapply(data, function(x) x[,4]))) # Mean AIC
      # new.results[r,3,avg] <- mean(unlist(lapply(data, function(x) x[,5]))) # Mean BIC
      new.results[r,4,avg] <- mean(unlist(lapply(data, function(x) (x[,2]-x[,3])^2)))^0.5 # rmse
      r <- r+1
      if(r>dim(new.results)[1]) r <- 1
  }
}
...
# for(i in 2) {
  ...
  while(previous.rmse >= current.rmse) {
    ...
    rsq <- unlist(lapply(combo.results.ls, function(combo) mean(unlist(lapply(combo, function(trial) trial[,1])) )))
    rmse <- unlist(lapply(combo.results.ls, function(combo) mean(unlist(lapply(combo, function(trial) (trial[,2]-trial[,3])^2)) )^0.5))
    all.combos[[length(all.combos)+1]] <- combo.results.ls
    # Save results
    results.ls[[length(results.ls)+1]] <- list(combos, rsq, rmse)
  ...
  all.combo.results.ls[[length(all.combo.results.ls)+1]] <- all.combos
  all.results.ls[[length(all.results.ls)+1]] <- results.ls
  save(all.results.ls, file="results/temp-all-results-ls-ct.RData")
}

save(all.combo.results.ls, file="results/ct-var-all-combo-results.RData")
...
var.df.ls <- list()
all.results.ls <- list(all.results.ls)
i <- 1
...
plot(x=actual,
     y=predicted, 
     xlim=c(min(na.omit(c(actual, predicted))), max(na.omit(c(actual, predicted)))), 
     ylim=c(min(na.omit(c(actual, predicted))), max(na.omit(c(actual, predicted)))), 
...
```{r}
load("results/ecoli-ann-all-results-ls-v2.RData")
new.results <- array(data=NA, dim=c(length(all.results.ls), 4, 3))

r <- 1
for(avg in 1:3) {
  for(year in 1:length(all.results.ls)) {
      data <- all.results.ls[[year]][[avg]]
      new.results[r,1,avg] <- mean(unlist(lapply(data, function(x) x[,1]))) # Mean R2
      # new.results[r,2,avg] <- mean(unlist(lapply(data, function(x) x[,4]))) # Mean AIC
      # new.results[r,3,avg] <- mean(unlist(lapply(data, function(x) x[,5]))) # Mean BIC
      new.results[r,4,avg] <- mean(unlist(lapply(data, function(x) (x[,2]-x[,3])^2)))^0.5 # rmse
...
lagged.times <- sapply(index(ecoli), function(t) {
  ...
  all.combo.results.ls[[i]][[length(all.combo.results.ls[[i]])+1]] <- all.combos
  all.results.ls[[length(all.results.ls)+1]] <- results.ls
...
```{r Var Ecoli exploratory, eval=FALSE, include=FALSE}
...
load("results/temp-all-results-ls-ecoli.RData")
# load("results/ecoli-var-results-ls-v2.RData")
var.df.ls <- list()
for(ecoli.i in 1:length(all.results.ls)) { # Pre = 1, post = 2
...
for(i in 1:(optimum.run+1)) {
  ...
  var.df[i,1] <- vars.key[optimum.combo,1]
  # var.df[i,2] <- all.results.ls[[ecoli.i]][[1]][[3]][which(all.results.ls[[ecoli.i]][[1]][[1]]==n)]
...
for(i in 1:2) {
  ...
  for(j in 1:length(process.data.ls)) {
    ...
    predict.col <- which(colnames(merged.data)==predict.var)
      ...
      resultsRNN.n <- list()
      for(n in 1:10) {
        resultsRNN.n[[length(resultsRNN.n)+1]] <- rollingRNN(all.data=merged.data,
                                                             train.obs = ceiling(nrow(merged.data)*0.9),
                                          predict.col=predict.col, 
                                          act.function="softsign", 
                                          n_epoch=2000,
                                          node_multiply=0.25, 
    ...
    colnames(merged.data)[ncol(merged.data)] <- "Lagged.ecoli"
      ...
      for(n in 1:10) {
        resultsRNN.n[[length(resultsRNN.n)+1]] <- rollingRNN(all.data=merged.data, predict.col, train.obs=ceiling(nrow(merged.data)*0.5),
                act.function="softsign", n_epoch=2000, node_multiply = 0.25)
      }
...
for(i in 1:2) {
  ...
  # Scale training data
   ...
   if(length(dim(train.x)) == 3) {
     ...
     if(is.null(n_nodes)) n_nodes <- c(dim(train.x)[2], round(dim(train.x)[2]*0.25))
