...
if("response-results-ls-001.RData" %in% list.files("results")) {
  load("results/response-results-ls-001.RData")
} else {
...
predict.col.name <- c("PAA...1.min..Sample",
                      "PAA...1.2.Basin.Sampling")

response.results.ls <- list()
for(iteration in 1:100) {
  
response.results <- list()
for(response in predict.col.name) {
  ...
  ## Prep data
  remove.column <- which(colnames(all.data) == predict.col.name[which(!(predict.col.name %in% response))])
  
  results <- foreach(i = 1:nrow(all.data), .combine = rbind) %dopar% {
    
    training.data <- all.data[-i,-remove.column]
    testing.data <- all.data[i,-remove.column]
    
    predict.column <- which(colnames(training.data) == response)
    
    ## Scale data using min-max method
    max <- apply(training.data, 2, max)
    min <- apply(training.data, 2, min)

    training.NN <- as.data.frame(scale(training.data, center = min, scale = max - min))
    testing.NN <- as.data.frame(scale(testing.data, center = min, scale = max - min))
    
    fmla <- as.formula(paste0(colnames(training.data)[predict.column],"~",
                       paste(colnames(training.data)[-predict.column], collapse= "+")))
    
    nodes <- 3
    NN <- neuralnet::neuralnet(fmla, training.NN, hidden = nodes, linear.output = FALSE)
    predict.NN <-  neuralnet::compute(NN, testing.NN)
    predict.NN <- (predict.NN$net.result * (max[predict.column] - min[predict.column])) + min[predict.column]
  
    return(data.frame(testing.data[,predict.column], # Actual
  ...
  response.results[[length(response.results)+1]] <- xts(results, order.by=as.POSIXct(rownames(results)))
}
response.results.ls[[length(response.results.ls)+1]] <- response.results
}
save(response.results.ls, file="./results/response-results-ls-001.RData")
}
```

```{r}
t <- cbind(130.5293*all.data$N..Basin.Outfall^-0.9495,
           1477.2*all.data$N..Basin.Outfall^-.958,
           2775*all.data$N..Basin.Outfall^-.959)
prediction.results.ls <- list()
for(iteration in 1:100) {
response.results <- response.results.ls[[iteration]]  
C1.prediction.e <- rep(NA, nrow(response.results[[1]]))
C2.prediction.e <- rep(NA, nrow(response.results[[1]]))
Ct.prediction.e <- rep(NA, nrow(response.results[[1]]))
for(i in 1:nrow(response.results[[1]])) {
  C_a <- c(as.numeric(response.results[[1]][i,1]), 
           as.numeric(response.results[[2]][i,1]))
  C_p <- c(as.numeric(response.results[[1]][i,2]), 
           as.numeric(response.results[[2]][i,2]))
  t_i <- as.numeric(t[i,])
  
  
  exponential.model <- lm(log(C_a) ~ t_i[1:2])
  k <- -1*exponential.model$coefficients[2]
  C0 <- exp(exponential.model$coefficients[1])
  Ct_a <- as.numeric(C0)/as.numeric(k)-as.numeric(C0)/as.numeric(k)*exp(-as.numeric(k)*t_i[3]) # Ct under single exponential curve
  
  exponential.model <- lm(log(C_p) ~ t_i[1:2])
  k <- -1*exponential.model$coefficients[2]
  C0 <- exp(exponential.model$coefficients[1])
  Ct_p <- as.numeric(C0)/as.numeric(k)-as.numeric(C0)/as.numeric(k)*exp(-as.numeric(k)*t_i[3]) # Ct under single exponential curve

  C1.prediction.e[i] <- (C_a-C_p)[1]
  C2.prediction.e[i] <- (C_a-C_p)[2]
  Ct.prediction.e[i] <- Ct_a-Ct_p
  
}

prediction.results.ls[[length(prediction.results.ls)+1]] <- list(C1.prediction.e, C2.prediction.e, Ct.prediction.e)
...
```{r Plot 2018 errors 1, echo=FALSE}
layout(mat=matrix(data=c(1,2,3), nrow=1))
C1.prediction.e <- do.call("c", lapply(prediction.results.ls, function(x) x[[1]]))
hist(C1.prediction.e, main="1-minute error", xlab="PAA (mg/L)")
mtext(paste("RMSE =",round(mean(C1.prediction.e^2)^0.5,2)), side=3,cex = 0.7, line=-0.5)
C2.prediction.e <- do.call("c", lapply(prediction.results.ls, function(x) x[[2]]))
hist(C2.prediction.e, main="1/2 basin error", xlab="PAA (mg/L)")
mtext(paste("RMSE =",round(mean(C2.prediction.e^2)^0.5,2)), side=3,cex = 0.7, line=-0.5)
Ct.prediction.e <- do.call("c", lapply(prediction.results.ls, function(x) x[[3]]))
hist(Ct.prediction.e, main="Ct error", xlab="PAA*t (mg/L min)")
mtext(paste("RMSE =",round(mean(Ct.prediction.e^2)^0.5,2)), side=3,cex = 0.7, line=-0.5)
```
...
if("response-results-ls-002.RData" %in% list.files("results")) {
  load("results/response-results-ls-002.RData")
} else {
...
for(response in predict.col.name) {
  ...
  results <- foreach(i = 1:nrow(all.data), .combine = rbind) %dopar% {
    ...
    #                    paste(colnames(training.data)[-predict.column], collapse= "+")))
    fmla <- as.formula(paste0(colnames(training.data)[predict.column],"~",
                   paste(colnames(training.data)[-c(predict.column, 8:12)], collapse= "+")))
...
save(response.results.ls, file="./results/response-results-ls-002.RData")
}
```
...
if("response-results-ls-003.RData" %in% list.files("results")) {
  load("results/response-results-ls-003.RData")
} else {
...
for(response in predict.col.name) {
  ...
  results <- foreach(i = 1:nrow(all.data), .combine = rbind, .packages = c("dplyr", "keras")) %dopar% {
    ...
    model <- keras_model_sequential() %>%
          layer_dense(units=nodes, 
                     input_shape=ncol(training.data)-1, 
                     activation = "softsign")
    ...
    history <- model %>% fit(
            x=data.matrix(training.NN[,-predict.column]),
            y=data.matrix(training.NN[,predict.column]),
            shuffle=TRUE,
            epochs = 100,
            verbose = 0
          )
    predict.NN <- model %>% predict(
          x=data.matrix(testing.NN[,-predict.column])
        )
    predict.NN <- (predict.NN * (max[predict.column] - min[predict.column])) + min[predict.column]
  ...
  response.results[[length(response.results)+1]] <- results
}
response.results.ls[[length(response.results.ls)+1]] <- response.results
}
save(response.results.ls, file="./results/response-results-ls-003.RData")
}
```
...
t <- cbind(130.5293*all.data$N..Basin.Outfall^-0.9495,
           ...
           2775*all.data$N..Basin.Outfall^-.959)
# Convert to xts?
# xts(results, order.by=as.POSIXct(rownames(results)))
prediction.results.ls <- list()
for(iteration in 1:length(response.results.ls)) {
response.results <- response.results.ls[[iteration]]  
C1.prediction.e <- rep(NA, nrow(response.results[[1]]))
C2.prediction.e <- rep(NA, nrow(response.results[[1]]))
Ct.prediction.e <- rep(NA, nrow(response.results[[1]]))
...
```{r Table north variables, eval=FALSE, include=FALSE}
...
```{r Table sorth variables, eval=FALSE, include=FALSE}
...
```{r South Predisinfection E.coli model fit, eval=FALSE, include=FALSE}
...
```{r North Predisinfection E.coli model fit, eval=FALSE, include=FALSE}
...
```{r South Predisinfection E.coli model performance, eval=FALSE, include=FALSE}
load(file=file.path(wd.path,"results","Over-under fit","average-log-lag.RData"))
test.average <- obj;rm(obj)
l <- lapply(test.average, function(results.list) {
  unlist(lapply(results.list, function(results) {
...
```{r North Predisinfection E.coli model performance, eval=FALSE, include=FALSE}
load(file=file.path(wd.path,"results","Over-under fit","average-log-lag-north.RData"))
test.average <- obj;rm(obj)
l <- lapply(test.average, function(results.list) {
  unlist(lapply(results.list, function(results) {
