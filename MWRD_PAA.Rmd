...
# Methods
## Linear Regression
In simple linear regression, a model is constructed of a response variable (**Y**) that is a linear function of other variables (**X_i**). The linear regression model assumes that **Y** is normally distributed, errors are normally distributed and independent, and **X** has constant variance. 
...
png("MWRD_Ecoli_vis_lm-COD.png", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 10)
...
par(mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
plot(x = data1[,1], y = fitted(lm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ AB.10.MLR.Flow, data = data1)), 
     xlim=c(8,11), ylim=c(8,11),
     xlab="Actal Log(N)", ylab="Predicted Log(N)", pch=20)
abline(a=0,b=1,col="blue", lwd=2)
text(x=8,y=11,labels="(a)", font=2)
...
png("MWRD_Ecoli_nseconline_lm-mlr.png", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 10)
par(mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
visreg::visreg(fit=(lm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ AB.10.MLR.Flow, data = data1)), xvar="AB.10.MLR.Flow", plot=TRUE, xlab = "AB-10 MLR Flow (MGD)", ylab = "", points.par=c(col="black"), line.par=c(col="blue"))
text(x=min(data1$AB.10.MLR.Flow),y=11,labels="(b)", font=2)
text(x=max(data1$AB.10.MLR.Flow),y=8.1,labels=round(summary.lm(lm)$r.squared,3), font=2, adj = 1)
...
```
<div custom-style="FigureCenter">
![](MWRD_Ecoli_vis_lm-COD.png)
![](MWRD_Ecoli_nseconline_lm-mlr.png)
</div>

**Figure 2.** Multiple linear regression model fit for pre-disinfection *E. coli*. Black circles represent actual observations. R-squared value in lower right. (a) Variation of predicted pre-disinfection *E. coli* given a range of CODds measured by a sensor near the sampling point (b) Variation of predicted pre-disinfection *E. coli* given a range of mixed liquor return (MLR) flows for AB-10 which provides influent to the disinfection basin.
...
par(mfrow=c(round(ncol((glm.nsec.online.ecoli$data)-1)/2)))
visreg(glm.nsec.online.ecoli )
```
