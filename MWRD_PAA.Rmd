...
scaled.boxplot.png <- function(data, filename, lines = seq(round(ncol(data)/5),ncol(data)-1, by = round(ncol(data)/5)) + .5, margins = c(9,3.5,0.5,0.5)+.1) {
  labs <- colnames(data)
  windowsFonts(A = windowsFont("Times New Roman"))
  png(filename, units = "in", res = 200, width = 6.5, height = 4, family="A", pointsize = 8)
  par(mar = margins)
...
# Goals
Design a PAA disinfection dosing system that account for:
...
nsec.online <- as.data.frame(suppressWarnings(read_excel("North Secondary and Disinfection Process Data_20190215.xlsx", sheet = "NSEC Online Data", col_names = FALSE,
...
# Remove outliers, such as negative flows, unrealistic sensor values
...
nsec.online <- nsec.online["2018-11-04/2018-12-01"] # Effluent NO5 invalid prior to June
...
# dev.off()
scaled.boxplot.png(nsec.online, "MWRD_nsec_online_boxplots.png")
```
...
n.paa.online <- as.data.frame(read_excel("North Secondary and Disinfection Process Data_20190215.xlsx", 
...
# Create boxplots
scaled.boxplot.png(n.paa.online, "MWRD_npaa_online_boxplots.png", margins = c(13,6.5,0.5,0.5)+.1)
```
...
# Create boxplots
# labs <- colnames(n.paa.grab)
# png("MWRD_npaa_grab_boxplots.png", units = "in", res = 200, width = 2*6.5, height = 2*4)
# par(mar = c(10,4.5,2,2)+.1)
# boxplot.data <- sapply(data.frame(nsec.online), function(x) scale(na.omit(x)))
# boxplot(boxplot.data, xaxt="n", xlab="", ylab = "Standard Deviations")
# text(x=seq_along(labs), y=par("usr")[3] - 1, srt=45, adj = 1, labels= labs, xpd=TRUE)
# dev.off()
scaled.boxplot.png(n.paa.grab, "MWRD_npaa_grab_boxplots.png", margins = c(12,6.5,0.5,0.5)+.1)
```


To achieve the goal of predicting *E. coli*, a log-transformed pre-disinfection *E. coli* grab data was merged with a reduced North secondary dataset (Figure 1). For simplicity, sensors monitoring SRT and effluent water quality of the North secondary system were used.<div custom-style="SpanCol">However, the residence time is need to appropriately merge datasets.</div>
...
scaled.boxplot.png(new.data, "MWRD_Ecoli_nseconline_boxplots.png", margins = c(12.5,6.5,0.5,0.5)+.1)
```
<div custom-style="FigureCenter">
![](MWRD_Ecoli_nseconline_boxplots.png)
</div>
**Figure 1.** Boxplots of water quality variables used to predict *E. coli*

# Methods
## Regression Models
```{r Linear regression, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
colnames(new.data) <- make.names(colnames(new.data))

lm.new.data <- lm(new.data[,1] ~ new.data[,2] + new.data[,3] + new.data[,4] + new.data[,5] + new.data[,6] + new.data[,7] + new.data[,8] + new.data[,9] + new.data[,10] + new.data[,11] + new.data[,12] + new.data[,13])
summary(lm.new.data)

lm.new.data <- lm(new.data[,1] ~ new.data[,2] + new.data[,3] + new.data[,4] + new.data[,5] + new.data[,6] + new.data[,7] + new.data[,9] + new.data[,11] + new.data[,12] + new.data[,13])
summary(lm.new.data)
which(summary.lm(lm.new.data)$coefficients[,4] == max(summary.lm(lm.new.data)$coefficients[,4]))


# Residuals:
#      Min       1Q   Median       3Q      Max 
# -1.29737 -0.42672 -0.09845  0.31344  2.19729 
# 
# Coefficients:
#                 Estimate Std. Error t value Pr(>|t|)    
# (Intercept)     5.955621   1.317663   4.520 9.27e-06 ***
# new.data[, 2]   0.023255   0.024729   0.940    0.348    
# new.data[, 3]  -0.020961   0.018343  -1.143    0.254    
# new.data[, 4]  -0.049539   0.058173  -0.852    0.395    
# new.data[, 5]   0.008722   0.020318   0.429    0.668    
# new.data[, 6]   0.005328   0.005858   0.910    0.364    
# new.data[, 7]  -0.003048   0.002069  -1.473    0.142    
# new.data[, 8]  -0.002595   0.006276  -0.413    0.680    
# new.data[, 9]  -0.008060   0.006861  -1.175    0.241    
# new.data[, 10]  0.112161   0.054901   2.043    0.042 *  
# new.data[, 11] -0.101265   0.089733  -1.129    0.260    
# new.data[, 12]  0.018478   0.043241   0.427    0.669    
# new.data[, 13]  0.102015   0.012201   8.361 3.34e-15 ***
# ---
# Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
# 
# Residual standard error: 0.5925 on 270 degrees of freedom
# Multiple R-squared:  0.4395,	Adjusted R-squared:  0.4146 
# F-statistic: 17.64 on 12 and 270 DF,  p-value: < 2.2e-16

# Model optimized by removing largest p-value until R-squared declined
# lm.new.data <- lm(new.data[,1] ~ new.data[,2] + new.data[,4] + new.data[,5] + new.data[,7] + new.data[,8])
lm.new.data <- lm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Effluent.NO5 + Quad.4.RAS.Flow + AB.10.Influent.Flow, data=new.data)
summary(lm.new.data)

# Residuals:
#      Min       1Q   Median       3Q      Max 
# -1.30175 -0.40165 -0.08242  0.31687  2.27927 
# 
# Coefficients:
#                      Estimate Std. Error t value Pr(>|t|)    
# (Intercept)          6.279084   0.428650  14.649  < 2e-16 ***
# NSEC.Effluent.NO5   -0.003394   0.001658  -2.048  0.04154 *  
# Quad.4.RAS.Flow      0.097714   0.010917   8.951  < 2e-16 ***
# AB.10.Influent.Flow  0.035839   0.013487   2.657  0.00833 ** 
# ---
# Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
# 
# Residual standard error: 0.5904 on 279 degrees of freedom
# Multiple R-squared:  0.4249,	Adjusted R-squared:  0.4187 
# F-statistic:  68.7 on 3 and 279 DF,  p-value: < 2.2e-16

# tiff("MWRD_Ecoli_nseconline_lm.tiff", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 8, compression = "none")
# par(mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
# plot(x = new.data[,1], y = fitted(lm.new.data), xlim=c(8,11), ylim=c(8,11), xlab="Actal Log(N)", ylab="Predicted Log(N)", pch=20)
# abline(a=0,b=1,col="blue", lwd=2)
# dev.off()

png("MWRD_Ecoli_nseconline_lm.png", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 10)
par(mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
plot(x = new.data[,1], y = fitted(lm.new.data), xlim=c(8,11), ylim=c(8,11), xlab="Actal Log(N)", ylab="Predicted Log(N)", pch=20)
abline(a=0,b=1,col="blue", lwd=2)
text(x=min(new.data[,1]),y=max(fitted(lm.new.data)),labels="(a)", font=2)
dev.off()


png("MWRD_Ecoli_nseconline_lm-ras.png", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 10)
par(mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
visreg::visreg(fit=lm.new.data, xvar="Quad.4.RAS.Flow", plot=TRUE, xlab = "Quad 4 RAS Flow (MGD)", ylab = "Pre-disinf E. coli (LOG MPN/100mL)", points.par=c(col="black"), line.par=c(col="blue"))
text(x=4,y=max(fitted(lm.new.data)),labels="(b)", font=2)
dev.off()

png("MWRD_Ecoli_nseconline_lm-no5.png", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 10)
par(mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
...
```
<div custom-style="FigureCenter">
![](MWRD_Ecoli_nseconline_lm.png)![](MWRD_Ecoli_nseconline_lm-ras.png)
</div>

**Figure 2.** Multiple linear regression model fit for pre-disinfection *E. coli*. Black circles represent actual observations. (a) Predicted pre-disinfection *E.coli* is plotted against actual observations. The blue line represents a perfect model fit. (b) Variation of predicted pre-disinfection *E. coli* given a range of aerobic solid retention times (SRTs) for north secondary.


```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
glm.new.data <- glm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Aerobic.SRT + NSEC.Effluent.OP + NSEC.Effluent.NO3 + NSEC.Effluent.NO5 + NSEC.Effluent.Flow, data=new.data)
visreg(glm.new.data)
```
...
## Figures
![](MWRD_nsec_online_boxplots.png)
**Figure S1.** Centered and scaled boxplots of north secondary online data from MWRD

![](MWRD_npaa_online_boxplots.png)
**Figure S2.** Centered and scaled boxplots of north disinfection online data from MWRD

![](MWRD_npaa_grab_boxplots.png)
**Figure S3.** Centered and scaled boxplots of north disinfection grab sample data from MWRD
