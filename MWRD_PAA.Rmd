...
scaled.boxplot.png(new.data[,-1], "MWRD_Ecoli_nseconline_boxplots.png", margins = c(7.5,4.5,0.5,0.5)+.1)
```
<div custom-style="FigureCenter">
![](MWRD_Ecoli_nseconline_boxplots.png)
</div>
**Figure 1.** Boxplots of water quality variables from North secondary used to predict *E. coli* from 2018-06-01 / 2018-12-01.

```{r Vis/Ecoli, message=FALSE, warning=FALSE, eval=FALSE, include=FALSE, paged.print=FALSE}
# Add Vis data?
...
runLinearRegression <- function(data) {
  data1 <- data
  ...
  while (summary.lm(lm.data1)$coefficients[,4][col2remove] > 0.1) {
    data1 <- data1[,-col2remove]
      fmla <- as.formula(paste0(colnames(data1)[1],"~", paste(colnames(data1)[2:length(colnames(data1))], collapse= "+")))
    lm.data1 <- lm(fmla, data = data1)
    col2remove <- which(summary.lm(lm.data1)$coefficients[,4] == max(summary.lm(lm.data1)$coefficients[,4]))
  }
...
lm <- runLinearRegression(data1)

png("MWRD_Ecoli_vis_lm.png", units = "in", res = 200, width = 3, height = 3, family="A", pointsize = 10)
...
text(x=min(data1[,1]),y=max(fitted(lm)),labels="(a)", font=2)
dev.off()
