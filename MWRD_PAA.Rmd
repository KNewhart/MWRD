...
scaled.boxplot.png <- function(data, filename, lines = seq(round(ncol(data)/5),ncol(data)-1, by = round(ncol(data)/5)) + .5, margins = c(9,3.5,0.5,0.5)+.1) {
  labs <- colnames(data)
  windowsFonts(A = windowsFont("Times New Roman"))
  png(filename, units = "in", res = 200, width = 2*6.5, height = 2*4, family="A", pointsize = 16)
  par(mar = margins)
...
To achieve the goal of predicting *E. coli*, a log-transformed pre-disinfection *E. coli* grab data was merged with a reduced North secondary dataset (Figure 1). For simplicity, sensors monitoring SRT and effluent water quality of the North secondary system were used. 
...
cols2keep <- c("NSEC Aerobic SRT", "NSEC Effluent NH3", "NSEC Effluent OP", "NSEC Effluent NO3","NSEC Effluent TSS","NSEC Effluent NO5","NSEC Effluent Flow")
...
scaled.boxplot.png(new.data, "MWRD_Ecoli_nseconline_boxplots.png", margins = c(13,6.5,0.5,0.5)+.1)
```
...
# [1] "Pre-disinf E. coli (LOG MPN/100mL)"
# [2] "NSEC Aerobic SRT"                  
# [3] "NSEC Effluent NH3"                 
# [4] "NSEC Effluent OP"                  
# [5] "NSEC Effluent NO3"                 
# [6] "NSEC Effluent TSS"                 
# [7] "NSEC Effluent NO5"                 
# [8] "NSEC Effluent Flow"

# lm.new.data <- lm(new.data[,1] ~ new.data[,2] + new.data[,3] + new.data[,4] + new.data[,5] + new.data[,6] + new.data[,7] + new.data[,8])
# summary(lm.new.data)
...
lm.new.data <- lm(Pre.disinf.E..coli..LOG.MPN.100mL. ~ NSEC.Aerobic.SRT + NSEC.Effluent.OP + NSEC.Effluent.NO3 + NSEC.Effluent.NO5 + NSEC.Effluent.Flow, data=new.data)
# summary(lm.new.data) 
# 
# Coefficients:
#                Estimate Std. Error t value Pr(>|t|)    
# (Intercept)    9.395790   0.233048  40.317   <2e-16 ***
# new.data[, 2]  0.055783   0.026385   2.114   0.0354 *  
# new.data[, 4] -0.077740   0.058613  -1.326   0.1858    
# new.data[, 5] -0.033610   0.019956  -1.684   0.0933 .  
# new.data[, 7] -0.012828   0.001438  -8.920   <2e-16 ***
# new.data[, 8]  0.002372   0.002069   1.146   0.2526    
# ---
# Signif. codes:  
# 0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 0.6605 on 279 degrees of freedom
# Multiple R-squared:  0.2825,	Adjusted R-squared:  0.2696 
# F-statistic: 21.96 on 5 and 279 DF,  p-value: < 2.2e-16

png("MWRD_Ecoli_nseconline_lm.png", units = "in", res = 200, width = 2*3, height = 2*3, family="A", pointsize = 16)
par(mar=c(4,4,.5,.5))
plot(x = new.data[,1], y = fitted(lm.new.data), xlim=c(8,11), ylim=c(8,11), xlab="Actal Log(N)", ylab="Predicted Log(N)")
abline(a=0,b=1,col="blue", lwd=2)
dev.off()

png("MWRD_Ecoli_nseconline_lm-srt.png", units = "in", res = 200, width = 2*3, height = 2*3, family="A", pointsize = 16)
par(mar=c(4,4,.5,.5))
visreg::visreg(fit=lm.new.data, xvar="NSEC.Aerobic.SRT", plot=TRUE, xlab = "North Secondary Aerobic SRT (d)", ylab = "Pre-disinf E. coli (LOG MPN/100mL)", points.par=c(col="black"), line.par=c(col="blue"))
dev.off()
# 
# visreg2d(fit=lm.new.data, "NSEC.Aerobic.SRT", "NSEC.Effluent.NO5", plot.type="image")
```
![](MWRD_Ecoli_nseconline_lm.png)
**Figure .** Multiple linear regression model fit for pre-disinfection *E. coli*. Black circles represent actual observations (x-axis) plotted against the prediction (y-axis). Blue line represents perfect model fit.

![](MWRD_Ecoli_nseconline_lm-srt.png)
**Figure .** Multiple linear regression model predicting pre-disinfection *E. coli*.
