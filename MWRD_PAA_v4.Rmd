...
```{r NN procedure, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
# NN
NNopt <- function(all.data, predict.col.name, percent.train = 0.8) {
  ## Prep data
...
for(i in 1:10) {
  NN.obj <- NNopt(all.data = all.data, predict.col.name = "Initial.PAA.Demand.or.Decay")
...
x <- data.frame(combn(seq(4,ncol(all.data)),7))
...
for(i in 1:ncol(x)) {
  NN.obj <- NNopt(all.data = all.data[,c(1:3,x[,i])], predict.col.name = "Initial.PAA.Demand.or.Decay")
  if(i==4) RMSE.i <- data.frame("Iteration" = i, 
                                "Col Removed" = colnames(all.data)[i],
                                "Hidden Nodes" = ncol(NN.obj[[1]]$weights[[1]][[1]]),
                                "RMSE" = NN.obj[[4]])
  if(i!=4) RMSE.i <- rbind(RMSE.i, data.frame("Iteration" = i, 
                                              "Col Removed" = colnames(all.data)[x[,i]],
                                              "Hidden Nodes" = ncol(NN.obj[[1]]$weights[[1]][[1]]),
                                              "RMSE" = NN.obj[[4]])
                           )
}
...
for (i in 1:nrow(RMSE.i_avg)) {
  RMSE.i_avg[i,4] <- mean(unlist(lapply(RMSE.i_ls, function(x) x[i,4]))) # Average RMSE of 1st row
}
RMSE.i_avg <- RMSE.i_avg[,-c(1,3)]
