...
knitr::opts_knit$set(root.dir = "C:\\Users\\KNewhart\\Documents\\GitHub\\MWRD")
...
lm.optimize <- function(data1) {
  if(!is.data.frame(data1)) data1 <- data.frame(data1)
  ...
  col2remove <- which.max(summary.lm(lm.data1)$coefficients[,4])

  while ((summary.lm(lm.data1)$coefficients[,4][col2remove] > 0.25) &&  (ncol(data1) >2)) {
...
lm.test <- lm.optimize(vis.ecoli.data)
...
lm.test <- lm.optimize(nsec.online.ecoli.data)
...
data1 <- cbind(vis.paa.data[,2:ncol(vis.paa.data)],vis.paa.data[,1])
lm.test <- lm.optimize(data1)

png("images/PAA_lm.png", res = 2000, units = "in", width = 4, height = 4)
par(mar=c(4,4,.5,.5))
visreg::visreg(fit=lm(Upstream.Residual.PAA..mg.L. ~ Initial.PAA.dose..mg.L., data=data1))
text(x=1.5, y=max(data1$Upstream.Residual.PAA..mg.L.), labels = "y = 0.6781x - 0.2107", adj = c(0, NA))
text(x=max(data1$Initial.PAA.dose..mg.L.), y = min(data1$Upstream.Residual.PAA..mg.L.), labels = "R-squared = 0.55", adj = c(1, NA))
dev.off()
```
<div custom-style="FigureCenter">
...
glm.optimize <- function(data1) {
  ...
  glm.data1 <- glm(fmla, data = data1)

  # r2 <- summary.glm(glm.data1)$r.squared

  col2remove <- which.max(summary.glm(glm.data1)$coefficients[,4])

  while ((summary.glm(glm.data1)$coefficients[,4][col2remove] > 0.25) &&  (ncol(data1) >2)) {
    data <- data1[,-col2remove]
    fmla <- as.formula(paste0(colnames(data)[1],"~", paste(colnames(data)[2:length(colnames(data))], collapse= "+")))
    glm.data <- glm(fmla, data = data)
    # if (summary.lm(lm.data)$r.squared < r2) {
    # break
    # } else {
      data1 <- data
      glm.data1 <- glm.data
      # r2 <- summary.lm(lm.data1)$r.squared
      col2remove <- which(summary.glm(glm.data1)$coefficients[,4] == max(summary.lm(glm.data1)$coefficients[,4]))
    # }
  }
...
glm.nsec.online.ecoli <- glm.optimize(nsec.online.ecoli.data)
...
glm.vis.ecoli.data <- glm.optimize(vis.ecoli.data)
...
```{r Random Forest and SVM, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
par(default.par)
...
sapply(colnames(data1)[2:length(colnames(data1))], function(x) visreg(fit=svm.test, xvar=x, plot=TRUE, xlab = x, ylab = "Pre-disinf E. coli (LOG MPN/100mL)", points.par=c(col="black"), line.par=c(col="blue"), 
       # xlim=c(20,100),
       ylim=c(8,11)))
...
sapply(colnames(data1)[2:length(colnames(data1))], function(x) visreg(fit=svm.test, xvar=x, plot=TRUE, xlab = x, ylab = colnames(data1)[1], points.par=c(col="black"), line.par=c(col="blue")))

for(i in 2:length(colnames(data1))) {
  x <- colnames(data1)[i]
  visreg(fit=svm.test, xvar=x, plot=TRUE, xlab = x, ylab = colnames(data1)[1], points.par=c(col="black"), line.par=c(col="blue"))
}
...
# n.plots <- as.numeric(which(summary(pca.nsec.online.ecoli)$importance[3,] > .9)[1]) - 1
# if((n.plots %% 2) == 1) n.plots <- n.plots + 1
# png("images/MWRD_Ecoli_nseconline_pca_all.png", units = "in", res = 2000, width = 6.5, height = 6.5, family="A", pointsize = 24)
# par(mfrow=c((n.plots/2),2))
# # for(i in seq(1,n.plots,2)){
#     fviz_pca_var(pca.nsec.online.ecoli, 
#                axes = c(i, (i+1)),
#                labelsize = 5,
#              col.var = "contrib",
#              gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
# pca.data <- pca.nsec.online.ecoli
pca.data <- prcomp(data.frame(data1), scale=TRUE)
major.loadings <- apply(pca.data$rotation,2, function(x) which(abs(x) > .5))
minor.loadings <- apply(pca.data$rotation,2,function(x) which((abs(x) > .25)))

major.loadings <- data.frame(unlist(lapply(major.loadings, function(x) do.call(paste, c(as.list(names(x)), sep = ", ")))))
major.minor.loadings <- data.frame(unlist(lapply(minor.loadings, function(x) do.call(paste, c(as.list(names(x)), sep = ", ")))))

View(major.loadings)
View(major.minor.loadings)
...
# data <- data.frame(nsec.online.ecoli.data)
data <- cbind(vis.paa.data[,2:ncol(vis.paa.data)],vis.paa.data[,1])

fmla <- as.formula(paste(colnames(data)[1],"~", do.call(paste, c(as.list(sapply(colnames(data)[2:length(colnames(data))], function(x) paste0("s(",x,")"))), sep = " + "))))

gam.test <- gam(fmla, data=data, method="REML")
summary(gam.test)
plot.gam(gam.test, ylab=colnames(data)[1], residuals = TRUE, ylim=c(-2,2))

fmla <- as.formula(paste(colnames(data)[1],"~", do.call(paste, c(as.list(sapply(colnames(data)[c(4,7)], function(x) paste0("s(",x,")"))), sep = " + "))))
gam.test <- gam(fmla, data=data, method="REML")
plot.gam(gam.test, ylab=colnames(data)[1], residuals = TRUE, ylim=c(-2,2))

# data.test <- data
# while(abs(e1071::skewness(data.test$UVT....)) > 0.5) {
#   rows2remove <- c(min(data$UVT), max(data$UVT....))
#   data.test <- data.test[-rows2remove,]
# }
# gam.test <- gam(fmla, data=data.test, method="REML")
# plot.gam(gam.test, ylab=colnames(data.test)[1], residuals = TRUE, ylim=c(-2,2))
...
gam.nsec.online.ecoli <- gam(fmla, data=data, method="REML")
summary(gam.nsec.online.ecoli)
plot.gam(gam.nsec.online.ecoli, ylab=colnames(data)[1], ylim=c(0,2), residuals=TRUE)
