...
# NNopt <- function(all.data, predict.col.name, percent.train = 0.8, training.index=NULL) {
  ...
  best.r2 <- which(data.avg == max(data.avg))
...
t1 <- 130.5293*test.data$Dis.North.Flow^-0.9495
```

# The online Carbovis analyzer 
```{r}
C2 <- test.data$PAA.Upstream.Residual
t2 <- 130.5293*test.data$Dis.North.Flow^-0.9495 + 10
```
