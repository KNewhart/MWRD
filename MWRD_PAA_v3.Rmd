...
# 1/2 basin PAA
calc.final <- predictor.flow - predict.NN

if(r==1) plot.data <- cbind(predictor.flow, calc.final)
if(r>1) plot.data <- cbind(plot.data, calc.final)
}

png("images/flow_variance_NN.png", units = "in", res = 2000, width = 5, height = 3.5)
par(cex=1, mar=c(3,3,.5,.5), mgp=c(1.75,.5,0))
plot(plot.data[,1:2], col="white", xlab = "PAA Dose (mg/L)", ylab = "1/2-basin PAA (mg/L)")
for(i in 2:ncol(plot.data)) {
  xspline(x=plot.data[,1], plot.data[,i], lty=2)
}
legend("bottomright",
       legend = c("Different Sets of Observed Conditions"),
       lty = c(2)
       )
dev.off()
```

```{r Impact of each variable}
...
png(paste0("images/onlinePAA_NN_",colnames(all.data)[predict.cols[predictor]],"_clean.png"), units="in", res=2000, width = 5, height = 4)
par(mar=c(3,3,2,8), mgp=c(1.75,.5,0))
plot(x = data2plot[,1], y = data2plot[,2]
     , xlim=c(min.val,max.val), ylim=c(min.val,max.val)
     , xlab="Actal", ylab="Predicted"
